...
# eval a string to a list/boolean (for cmd_list from 'bot settings' or
# DEBUG from config)
import ast
...
# How long a single user can go without being audited
audit_period = int(configs.get('bot settings', 'audit_period'))
# how often the BOT audits all users
audit_interval = int(configs.get('bot settings', 'audit_interval'))
client_restriction_limit = int(configs.get(
    'bot settings', 'client_restriction_limit'))
...
# Banner that we send to all users upon initialization (not in
# bot_messages.py because it requires some variables from this script)
bot_msg = '''
...
class Bot:
    ...
    def setGuildTags(self, client_id):
        for i in ts3conn.clientinfo(clid=client_id):
            # TODO: Finish setting guild tags, command disabled globaly for
            # now...
            print(i)
    ...
    def getUserDatabase(self):
        if os.path.isfile(self.db_name):
            self.db_conn = sqlite3.connect(
                self.db_name, check_same_thread=False, detect_types=sqlite3.PARSE_DECLTYPES)
            ...
            self.db_cursor.execute(
                'INSERT INTO bot_info (last_succesful_audit) VALUES (?)', (datetime.date.today(),))
            self.db_conn.commit()
    ...
    def TsClientLimitReached(self, gw_acct_name):
        ...
        client_exists = self.db_cursor.execute(
            "SELECT EXISTS(SELECT 1 FROM users WHERE account_name=?)",  (gw_acct_name,)).fetchone()
    ...
    def removeUserFromDB(self, client_db_id):
        self.db_cursor.execute(
            "DELETE FROM users WHERE ts_db_id=?", (client_db_id))
        self.db_conn.commit()
    ...
    def auditUsers(self):
        ...
        for audit_user in self.db_audit_list:
            ...
            if DEBUG:
                ...
                print("TODAY |%s|  LAST AUDIT |%s|" % (self.c_audit_date,
                                                       audit_last_audit_date + datetime.timedelta(days=audit_period)))
            ...
            if self.c_audit_date >= audit_last_audit_date + datetime.timedelta(days=audit_period):
                ...
                if auth.success:
                    TS3Auth.log("User %s is still on %s. Succesful audit!" % (
                        audit_account_name, auth.required_server))
                    self.db_cursor.execute(
                        "UPDATE users SET last_audit_date = ? WHERE ts_db_id= ?", (self.c_audit_date, audit_ts_id,))
                    self.db_conn.commit()
                else:
                    TS3Auth.log("User %s is no longer on %s. Removing access...." % (
                        audit_account_name, auth.required_server))
                    self.db_cursor.execute(
                        "DELETE FROM users WHERE ts_db_id= ?", (audit_ts_id))
                    self.db_conn.commit()
                    self.removePermissions(audit_ts_id)
        self.db_cursor.execute(
            'INSERT INTO bot_info (last_succesful_audit) VALUES (?)', (self.c_audit_date,))
        self.db_conn.commit()
...
# Restricts commands for the channel messages (can add custom ones). Also
# add relevant code to 'rec_type 2' in my_event_handler.
...
# Handler that is used every time an event (message) is received from
# teamspeak server
    ...
    try:
        ...
        with ts3.query.TS3Connection(host, port) as ts3conn:
            ...
            try:
                ...
                exit(1)
            # Force connection to stay up by sending an alive message every 250
            # seconds
            ts3conn.keepalive(interval=250)

            # Choose which server instance we want to join (unless multiple
            # exist the default of 1 should be fine)
            ts3conn.use(sid=server_id)

            # Define our bots info
            BOT = Bot(ts3conn.whoami(), db_file_name)
            TS3Auth.log("BOT loaded into server (%s) as %s (%s). Nickname '%s'" % (
                server_id, BOT.name, BOT.client_id, BOT.nickname))
            ...
            while verify_channel_id == 0:
                try:
                    ...
                    TS3Auth.log(
                        "Unable to locate channel with name '%s'. Sleeping for 10 seconds..." % (channel_name))
                    time.sleep(10)
            ...
            try:
                ...
                TS3Auth.log("BOT has joined channel '%s' (%s)." %
                            (channel_name, verify_channel_id))
            except ts3.query.TS3QueryError as chnl_err:  # BOT move failed because
                TS3Auth.log("BOT Attempted to join channel '%s' (%s) WARN: %s" % (
                    channel_name, verify_channel_id, chnl_err.resp.error["msg"]))

            ts3conn.servernotifyregister(
                event="textchannel")  # alert channel chat
            ts3conn.servernotifyregister(
                event="textprivate")  # alert Private chat
            ...
            ts3conn.sendtextmessage(
                targetmode=3, target=server_id, msg=bot_msg)
            TS3Auth.log("BOT is now registered to receive messages!")

            TS3Auth.log("BOT Database Audit policies initiating.")
            # Always audit users on initialize if user audit date is up (in
            # case the script is reloaded several times before audit interval
            # hits, so we can ensure we maintain user database accurately)
            BOT.auditUsers()

            # Set audit schedule job to run in X days
            schedule.every(audit_interval).days.do(BOT.auditUsers)

            # Forces script to loop forever while we wait for events to come
            # in, unless connection timed out. Then it should loop a new bot
            # into creation.
        ...
        TS3Auth.log(
            "It appears the BOT has lost connection to teamspeak. Trying to restart connection in %s seconds...." % bot_sleep_conn_lost)
        time.sleep(bot_sleep_conn_lost)

    except ConnectionRefusedError:
        TS3Auth.log(
            "Unable to reach teamspeak server..trying again in %s seconds..." % bot_sleep_conn_lost)
        time.sleep(bot_sleep_conn_lost)
